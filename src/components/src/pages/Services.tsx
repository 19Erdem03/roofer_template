...
import { Home, Wrench, Zap, Droplets, Search, Store as Storm, Shield, Award, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';

const Services = () => {
  const [servicesScrollState, setServicesScrollState] = useState({ canScrollLeft: false, canScrollRight: true });
  const [processScrollState, setProcessScrollState] = useState({ canScrollLeft: false, canScrollRight: true });

  const CARD_WIDTH = 320; // w-80 = 320px
  const GAP = 24; // space-x-6 = 24px
  const SCROLL_AMOUNT = CARD_WIDTH + GAP;

  const updateScrollState = (containerId: string, setState: React.Dispatch<React.SetStateAction<{ canScrollLeft: boolean; canScrollRight: boolean }>>) => {
  ...
  useEffect(() => {
    ...
    const processContainer = document.getElementById('process-carousel');

    const handleServicesScroll = () => updateScrollState('services-carousel', setServicesScrollState);
    const handleProcessScroll = () => updateScrollState('process-carousel', setProcessScrollState);

    if (servicesContainer) {
      servicesContainer.addEventListener('scroll', handleServicesScroll);
      // Initial state
      updateScrollState('services-carousel', setServicesScrollState);
    }

    if (processContainer) {
      processContainer.addEventListener('scroll', handleProcessScroll);
      // Initial state
      updateScrollState('process-carousel', setProcessScrollState);
    }
    ...
    return () => {
      ...
      if (processContainer) {
        processContainer.removeEventListener('scroll', handleProcessScroll);
      }
    };
  }, []);
    ...
    <div className="pt-16">
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:hidden relative">
            ...
            <div id="services-carousel" className="overflow-x-auto">
              <div className="flex space-x-6 pb-4 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              <style jsx>{`
              ...
              `}</style>
                {services.map((service, index) => (
                  <div key={index} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow flex-none w-80 snap-center">
                    <div className="p-8">
            ...
            <div className="text-center mt-6">
              <p className="text-gray-500 text-sm">← Scroll to see all services →</p>
            </div>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:hidden relative">
            {/* Navigation Arrows */}
            {processScrollState.canScrollLeft && (
              <button
                onClick={() => {
                  const container = document.getElementById('process-carousel');
                  if (container) {
                    container.scrollBy({ left: -SCROLL_AMOUNT, behavior: 'smooth' });
                  }
                }}
            ...
            {processScrollState.canScrollRight && (
              <button
                onClick={() => {
                  const container = document.getElementById('process-carousel');
                  if (container) {
            ...
            <div id="process-carousel" className="overflow-x-auto">
              <div className="flex space-x-6 pb-4 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                <style jsx>{`
                  div::-webkit-scrollbar {
                    display: none;
                ...
                {[
                  ...
                  <div key={index} className="text-center flex-none w-80 snap-center bg-gray-50 p-8 rounded-xl">
            ...
            {/* Navigation hint for mobile */}
            <div className="text-center mt-6">
              <p className="text-gray-500 text-sm">← Scroll to see all steps →</p>
            </div>
          </div>
